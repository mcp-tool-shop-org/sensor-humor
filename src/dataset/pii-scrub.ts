 ...
 * `tool`/`version` stamp makes a re-scrub with a better tool distinguishable at read time. Only
 * `user_input` rows are ever scrubbed — synthetic internal-seed rows carry no user PII (pii_scrub n/a).
 *
...
export const PII_SCRUB_VERSION = '0.1.0';
 ...
 * fails safe toward exclusion, and only user_input rows are scrubbed, so a false positive costs one
 * excluded row, never a leak.
 */
const DETECTORS: Record<PiiEntityClass, RegExp[]> = {
  ...
  key: [
    /\bsk-[A-Za-z0-9]{16,}\b/g, // stripe/openai-style secret key
